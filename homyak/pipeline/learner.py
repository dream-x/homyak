...
_REFINE_SYSTEM = (
    ...
    '{"description": "<на русском>", "topics": [{"name": "<тег>", "polarity": "love|like|mute"}]}.\n\n'
    "description — уточняй, а НЕ переписывай заново. Возьми текущий профиль за основу и сохрани его "
    "объём и глубину: роль и уровень человека, любимые жанры материалов, чем он занимается и что "
    "отвергает. Реакции добавляют акценты, а не заменяют портрет. Сокращать нельзя — краткая выжимка "
    "делает профиль хуже: судья читает описание дословно, и обеднённый текст режет целые темы.\n\n"
    "topics — ТОЛЬКО имена из словаря тегов, который дан ниже. Не придумывай составных названий: "
    "тега, которого нет в словаре, теггер не поставит никогда, и такая тема ни с чем не совпадёт.\n\n"
    "love/like — что он явно лайкает; mute — что явно дизлайкает."
)
...
async def maybe_refine(repo: NewsRepo, bus: NatsBus, llm: OllamaLLM, vertical: str) -> None:
    ...
    vocab = await repo.top_tags(vertical)
    user = (
        f"Вертикаль: {vertical}\nТекущий профиль: «{desc}»\n\n"
        f"Словарь тегов (только эти имена): {', '.join(vocab) or '—'}\n\n"
        f"Лайкал: {fmt(liked)}\n\nДизлайкал: {fmt(disliked)}\n\n"
        "Предложи уточнённый профиль (description, не короче текущего) и 8-16 тем с polarity."
    )
    try:
