...
class PrefilterAnalyzer:
    ...
    async def analyze(self, ctx: AnalyzerContext) -> None:
        ...
        # Ссылку принёс человек через бота — гейту тут нечего решать. Отсев необратим
        # (пути переобработки skipped нет), а «низкая близость к профилям» у сознательно
        # добавленной ссылки означает лишь то, что тема новая.
